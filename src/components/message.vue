  ...
  <div class="wrapper">
    ...
    <main>
      ...
      <ul v-if="accesstoken" class="lists" v-show="activeTab==='hasnot_read'">
        <router-link :to="{path:'/content',query:{id:item.topic.id}}" tag="li" class="list" v-for="item in hasnot_read_messages" :key="item.id">
          <div class="user">
      ...
      <ul v-if="accesstoken" class="lists" v-show="activeTab==='has_read'">
        <router-link :to="{path:'/content',query:{id:item.topic.id}}" tag="li" class="list" v-for="item in has_read_messages" :key="item.id">
          <div class="user">
            <img :src="item.author.avatar_url" alt="user">
            <span>{{item.author.loginname}}</span>
          </div>
...
.activeTab{
  position: relative;
  border-top: 1px solid rgbd(255,255,255,.1);
}
.count{
  position: absolute;
  left: 10%;
